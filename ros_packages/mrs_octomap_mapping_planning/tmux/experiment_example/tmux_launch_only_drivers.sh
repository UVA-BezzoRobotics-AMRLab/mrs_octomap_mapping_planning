...
input=(
  # 'Arming' 'waitForControl; rosservice call /$UAV_NAME/hw_api/arming 1; sleep 2; rosservice call /$UAV_NAME/hw_api/offboard
# '
  'GoTo'  'history -s rosservice call /$UAV_NAME/control_manager/goto \"goal: \[0.0, 0.0, 0.5, 0.0\]\"
'
...
'
  'Oak_Front' 'waitForHw; roslaunch mrs_uav_deployment oak_front.launch
'
  'Oak_Down'  'waitForHw; roslaunch mrs_uav_deployment oak_down.launch
'
  'Ouster' 'waitForTime; roslaunch ouster_ros uav.launch
'
  'Aloam'  'waitForTime; roslaunch mrs_aloam_core slam_pipeline.launch OUSTER_TYPE:=OS0-64
'
  'Mapping' 'waitForHw; roslaunch mrs_octomap_mapping_planning mapplan_real.launch config_octomap_server:=./config/octomap_server.yaml config_octomap_planner:=./config/octomap_planner.yaml config_pcl_filter_ouster:=./config/ouster_filter_mapping.yaml
'
  'WaypointPlanner' 'history -s roslaunch mrs_octomap_mapping_planning ps_octomap.launch
'
  # 'AutoStart' 'waitForHw; roslaunch mrs_uav_autostart automatic_start.launch

# '
  # 'RVIZ' 'waitForControl; rviz rviz -d `rospack find mrs_uav_deployment`/config/rviz/sensors.rviz
# '
